...
# Load the model with the weights_only=True parameter to avoid the security warning
...
def crop_prediction():
    try:
        ...
        # Debugging: Print collected inputs
        print(f"Inputs received - N: {N}, P: {P}, K: {K}, ph: {ph}, rainfall: {rainfall}, temperature: {temperature}, humidity: {humidity}")

        # Prepare the input data as a numpy array
        data = np.array([[N, P, K, temperature, humidity, ph, rainfall]])
        
        # Debugging: Print data shape and content
        print(f"Data prepared for prediction: {data}, Shape: {data.shape}")

        # Ensure the model is loaded and ready for prediction
        if crop_recommendation_model is None:
            raise ValueError("The crop recommendation model is not loaded.")
        ...
        return render_template('crop.html', prediction=final_prediction, title='Harvestify - Crop Recommendation')

    except Exception as e:
        # Log the error for debugging
        print(f"Error during prediction: {e}")
        
        # Render an error page with the specific error message
